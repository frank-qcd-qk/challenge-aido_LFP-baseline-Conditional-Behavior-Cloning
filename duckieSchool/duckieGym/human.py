...
class HumanDriver:
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.BACKSPACE or symbol == key.SLASH:
            ...
            self.env.reset()
            self.env.render()
            self.sleep_after_reset(5)  # modified reset time to 3 s
    ...
    def update(self, dt, env):
        ...
        if self.joystick.buttons[4]:
            x = -1.0
            z = 0.0

        # press RB (button 5) to indicate there is obstacles ahead
        obstacle = 0
        if self.joystick.buttons[5]:
            obstacle = 1
        ...
        if reward != -1000:
            ...
            if ((reward > self.last_reward - 0.02) or not self.filter_bad_data):
                ...
                step = Step(cropped_final, reward, action, done, obstacle)
        ...
        if done:
            # added feature, only do playback when enable_playback is true
